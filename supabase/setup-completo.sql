-- ============================================
-- ProjetoPCI - Setup Completo do Supabase
-- Execute este script no SQL Editor do Supabase
-- ============================================

-- PARTE 1: SCHEMA (Tabelas e Estrutura)
-- ============================================
...
CREATE TABLE usuarios (
  ...
  papel TEXT NOT NULL CHECK (papel IN ('Principal', 'Candidato', 'Líder')),
  nome TEXT NOT NULL,
  email TEXT NOT NULL,
...
-- Função auxiliar para pegar user_id do Clerk JWT
CREATE OR REPLACE FUNCTION get_user_id_from_clerk()
RETURNS UUID AS $$
BEGIN
  RETURN (
    SELECT id FROM usuarios
    WHERE clerk_id = auth.jwt()->>'clerk_id'
    LIMIT 1
  );
END;
...
CREATE POLICY "Anyone can insert" ON usuarios
  FOR INSERT WITH CHECK (true);

-- Policies para PERFIS_COMPORTAMENTAIS
...
CREATE POLICY "View functions of accessible processes" ON funcoes
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        processos.usuario_principal_id = get_user_id_from_clerk()
...
-- Policies para CANDIDATOS
CREATE POLICY "View candidates of accessible processes" ON candidatos
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM processos
    ...
    OR candidatos.usuario_id = get_user_id_from_clerk()
  );
...
-- Policies para PERGUNTAS_EMPRESA
...
-- Policies para RESPOSTAS_AUDIO
CREATE POLICY "View own audio responses" ON respostas_audio
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM candidatos
...
CREATE POLICY "Insert own audio responses" ON respostas_audio
...
-- Policies para FIT_ANALISES
CREATE POLICY "View fit analyses of accessible processes" ON fit_analises
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM processos
    ...
    OR EXISTS (
      SELECT 1 FROM candidatos
      WHERE candidatos.id = fit_analises.candidato_id
      AND candidatos.usuario_id = get_user_id_from_clerk()
    )
  );
